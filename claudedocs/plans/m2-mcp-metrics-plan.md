...
| B | パーサー拡張 | T05-T07 | 認知的複雑度, bodyHash, パーサーオーケストレータ更新 | pending |
| C | ファイル・関数レベルメトリクス | T08-T15 | 8つの新メトリクス実装 | pending |
| D | モジュール・アーキテクチャメトリクス | T16-T19 | 4つの新メトリクス実装 | pending |
...
Phase 3: T08-T19 (12メトリクス) ← 並列実装可能（各メトリクス独立）
Phase 4: T20-T22 (統合・テスト更新) ← 全メトリクス揃ってから
Phase 4.5: T22a-T22d (詳細出力) ← M1 details補完 + フォーマッタ拡張
Phase 5: T23-T29 (MCPサーバー) ← Phase 4完了後（Phase 4.5と並列可能）
Phase 6: T30-T32 (E2E・検証) ← 全体完成後
```

Phase 3のT08-T19は相互依存がないため並列実装可能。ただし、全てGroup AとGroup Bに依存するため、Phase 1-2の完了が前提。
